...
str(Cuckoo)
Cuckoo = data.frame(Cuckoo)
Cuckoo

ggplot(Cuckoo, aes(x=Bird, y=Length, fill=Bird)) +
  geom_boxplot(alpha = 0.3) +
  geom_jitter(aes(color=Bird), size=1.3, alpha=0.8, width = 0.1, shape =3) +
  theme_bw() +
  theme(
...
ggplot(data, aes(x=Bird, y=Length, fill=Bird)) +
  geom_boxplot(alpha = 0.3) +
  geom_jitter(aes(color=Bird), size=2, alpha=1, width = 0.1, shape=3) +
  scale_fill_manual(values = list("robin"="salmon","wren"="turquoise3")) +
  theme_bw() +
  theme(
    legend.position="none",
    plot.title = element_text(size=11)
